...
class CircularList {
	...
	{
		Node3 q=first, current = first.link;
...
//		SimpleObject3 so;
	...
	void Merge(CircularList b, Comparator<SimpleObject3> cc) {
		...
		Node3 p=first.link, q=b.first.link, prev=first;
		...
		if(p==first)	prev.link = q;
		
		while(q!=first) {
			if(q.link == b.first)	q.link = first;
			q = q.link;
		}
