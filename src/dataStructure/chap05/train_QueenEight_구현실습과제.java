...
import dataStructure.chap05.Stack.EmptyGenericStackException;
...
public class train_QueenEight_구현실습과제 {
	static void EightQueen(int[][] d) {
		...
		while (true) { // 교재 175 페이지 코드 사용해서 구현 필요
			if (st.isEmpty() && iy == 8) { // ix가 8이면 8개 배치 완료, stack이 empty가 아니면 다른 해를 구한다
				System.out.println("경우의 수 : "+ numberOfSolutions);
				break;
			}
	...
	public static boolean checkDiagSW(int[][] d, int cx, int cy) { // x++, y-- or x--, y++ where 0<= x,y <= 7
		int tmpX = cx + 1;
		int tmpY = cy - 1;
		while(tmpX<d.length && tmpY>=0) {
			if (d[tmpX++][tmpY--]==1) return false;
		}
		tmpX = cx - 1 ; 
		tmpY = cy + 1 ;
		while(tmpX>=0 && tmpY<d[0].length) {
			if(d[tmpX--][tmpY++]==1)	return false;
		}
		return true;
	}
	...
	public static boolean checkDiagSE(int[][] d, int cx, int cy) {// x++, y++ or x--, y--
		...
		tmpX = cx + 1;
		tmpY = cy + 1;
		while(tmpX<d.length && tmpY<d[0].length) {
			if(d[tmpX++][tmpY++]==1)	return false;
		}
		return true;
