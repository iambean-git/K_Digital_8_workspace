...
class LinkedList1 {
	...
	public void Show() { // 전체 리스트를 순서대로 출력한다.
		...
		int num = 0;
		while(p!=null) {
			System.out.println(num + "번째 요소 : " + p.data);
			num ++;
			p = p.link;
		}
	...
	public void Add(int element) // 임의 값을 삽입할 때 리스트가 오름차순으로 정렬이 되도록 한다
	{
		...
		Node1 p = first, q=null; //p:현재 노드, q: 이전 노드를 따라감
		while(p != null) {
			if(element > p.data) {
				q = p;
				p = p.link;
				if(p==null) {
					q.link = newNode;	//젤 마지막에 삽입 하는 경우
					return;
				}
			} else {
				newNode.link = p;	
				if(q == null) { 	//맨처음 삽입 되는 경우
					first = newNode;	
					return;
				}
	...
	void Merge(LinkedList1 b) {
		 ...
		 */
		Node1 p = first, q=b.first, prev=null;		// q: this용 인덱스, q: b용 인덱스
