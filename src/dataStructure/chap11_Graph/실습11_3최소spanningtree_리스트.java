...
class Graph_MST {
    ...
    public void insertEdge3(int src, int dest, int weight) {
        ...
        //adjacencyList[dest].add(new Edge3(dest, src, weight));
    }

    public void displayAdjacencyLists() {
        for (int i = 0; i < n; i++) {
            System.out.print(i + " -> ");
            for (Edge3 edge : adjacencyList[i]) {
...
public class 실습11_3최소spanningtree_리스트 {
    static void KruskalMST(Graph_MST graph) {
        ...
        // 모든 간선을 리스트에 추가
        for (int i = 0; i < n; i++) {
            for (Edge3 edge : graph.adjacencyList[i]) {
