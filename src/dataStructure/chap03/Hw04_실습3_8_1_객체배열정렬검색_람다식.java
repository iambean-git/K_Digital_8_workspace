...
class Fruit4 {
	...
	public String toString() {
		return  name + "	[price=" + price + ",	expire=" + expire + "]";
	}
...
public class Hw04_실습3_8_1_객체배열정렬검색_람다식 {
	...
	private static int binarySearch(Fruit4[] arr, Fruit4 keyF, Comparator<Fruit4> cc) {
		// TODO Auto-generated method stub
		//cc.compare(keyF, keyF) 활용
		int pl = 0;
		int pr = arr.length-1;
		
		while(pl<=pr) {
			int pc = (pl+pr)/2;
			if(cc.compare(arr[pc], keyF)==0)
				return pc;
			else if(cc.compare(arr[pc], keyF)<0)
				pl = pc + 1 ;
			else 
				pr = pc - 1 ;
		}
